...
> [!note]
> The printed representation of strings is different from the string itself. To see the row content of a string, use `str_vew()`.
...
When you habve many escapes `\` in a string, so calles [leaning toothpick syndrome](https://en.wikipedia.org/wiki/Leaning_toothpick_syndrome) you can use **raw strings**.
