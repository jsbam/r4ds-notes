...
`separate_wider_delim()` requires a fixed and known set of columns. If some rows don't have the expected number of pieces, the function has 2 arguments `too_few` and `too_many` to handle these problems.
